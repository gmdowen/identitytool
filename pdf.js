(function () {
  ...
  function drawCoverPage(doc, data) {
    ...
    doc.setFontSize(18);
    doc.setTextColor(...TEXT_DIM);
    const numWidth = doc.getTextWidth(String(data.results.identity));
    doc.text('/100', PAGE_W / 2 + numWidth / 2 + 3, y);
    ...
    doc.setTextColor(...TEXT_DIM);
    doc.setCharSpace(1.6);
    doc.text('YOUR ARCHETYPE', MARGIN, y);
    doc.setCharSpace(0);
    ...
    doc.setFontSize(30);
    doc.setTextColor(...GOLD);
    doc.text(data.archetype.name, MARGIN, y);

    y += 12;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(11);
    doc.setTextColor(...TEXT_SOFT);
    y = wrappedText(doc, data.archetype.desc, MARGIN, y, CONTENT_W, 5.6);
    ...
    doc.setTextColor(...TEXT_DIM);
    doc.setCharSpace(1.6);
    doc.text('COACHABILITY', MARGIN, y);
    doc.setCharSpace(0);
    ...
    doc.setFontSize(13);
    doc.setTextColor(...TEXT);
    doc.text(`${data.results.coach}/100`, MARGIN, y);
  }
  ...
  function drawFiveSignsPages(doc, data) {
    ...
    doc.setFontSize(20);
    doc.setTextColor(...GOLD);
    y = wrappedText(doc, "Come back when you are ready.", MARGIN, y, CONTENT_W, 8);

    y += 6;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(11);
    doc.setTextColor(...TEXT_SOFT);
    const closing = "Until then, stay up to date with both Instagram and YouTube, and I will continue to share more value for you to take action on yourself from there. I will keep meeting you where you are in the meantime.";
    y = wrappedText(doc, closing, MARGIN, y, CONTENT_W, 5.8);
    ...
    doc.textWithLink('Subscribe on YouTube', MARGIN + 60, y, { url: data.youtubeUrl });
  }
  ...
  // Read config values from quiz.js scope via DOM links if present, or fallback
  function getCalendlyUrl() {
    const a = document.querySelector('#cta-block a[href*="calendly"]');
    return a ? a.href : 'https://calendly.com/gareth-owen/identity-call';
  }
  function getInstagramUrl() {
    const a = document.querySelector('#cta-block a[href*="instagram"]');
    return a ? a.href : 'https://instagram.com/gareth.owen';
  }
  function getYoutubeUrl() {
    const a = document.querySelector('#cta-block a[href*="youtube"]');
    return a ? a.href : 'https://youtube.com/@gareth.owen';
  }
})();
